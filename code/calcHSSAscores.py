...
def main():
    ...
    db_fn =  '../query_results/sea_5km-tim.db'
    ...
    # print(getTabNames(db))
    # print(getColNames(db, 'orig'))
...
def calcWalkScores(cursor, db, max_dur):
    ...
    for i in range(orig_ids.shape[0]):
        ...
        for j in range(services_pd.shape[0]):
            ...
            # print(services_pd.ix[j, 'dest_id'])
            # print(orig_ids.ix[i, 'orig_id'])
    ...
    scores_pd['HSSAscore'] = scores_pd['HSSAscore'].divide(max(scores_pd['HSSAscore']))
