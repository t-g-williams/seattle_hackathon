...
parcels <- readOGR(dsn = file.path(dir.work,'data','WGS84'), layer = fn.parcels, verbose = FALSE)
...
# import iner zips
fn.cbd <- 'sea_innerzips'
cbd <- readOGR(dsn =  file.path(dir.work,'data'), layer = fn.cbd, verbose = FALSE)

# transform to meters
parcels <- spTransform(parcels, CRS(proj4string(city)))
cbd <- spTransform(cbd, CRS(proj4string(city)))
...
CreateNearTable <- function(parcels,  buf.dist){
  ...
  saveRDS(near.table, file = 'neartable_full.rds')# save compressed object
...
chunkNearTable <- function(chunk.orig.pts, i, dest.pts, buf.dist) {
  ...
  # save
  saveRDS(neartable.chunk, file = paste0('neartable_part_', i, '.rds'))
...
CreateNearTable(parcels,  buf.dist)